...
| Fig. S1 du supplément (convergence en n_O ; Fig. 3 du corps jusqu'au 19 sept., fichier `fig3_tiling_convergence`) | `t_op3.py`, `prod_op.py`, `t4_graded.py` → `make_figures_v2.py` | idem | 16 sept. ; renumérotée le 20 sept. |
...
| Fig. 3 du corps (balayage b₀/g ; fichier `fig3_b0g_sweep`) et dernier paragraphe de §IV-C : g seul varie (R_s fixe, R_r = R_s − g), b₀ = 2,000 mm aux deux surfaces, pavage (33,16), chapeau symétrique, N_h = 8192, cavités Q rechargées de `cavity_Q.pkl` ; b₀/g ∈ {2, 4, 8,2 (nominal relu de `prod_fem_results.json` / `prod_op_results.json`), 12, 16} ; à chaque g les douze positions rotor : EF formulation A (lc 0,045 ; lc 0,0225 à b₀/g = 12 et 16, voir la ligne suivante ; comparateur forme fermée recalculé), produit de Carter (formes approchée et conforme exacte), opérateur Φ_O = 0 et opérateur + cavités (comparateur assemblé recalculé) ; gardes G1–G5 dans l'en-tête du transcript (tolérance 0 sur les chiffres imprimés) | `prod_sweep_b0g.py` → `prod_sweep_b0g_meshrow.py` → `FIG3_B0G_SWEEP.py` | `prod_sweep_b0g_results.json`, `prod_sweep_b0g_out.txt` (Tables 1–2 : ligne b₀/g = 12 promue à lc 0,0225, ligne au maillage de production conservée et marquée) ; `code/article/figures/fig3_b0g_sweep.{pdf,png}` | 19–20 sept. |
| Vérification de maillage du balayage b₀/g : G5 de `prod_sweep_b0g.py` au g le plus fin (b₀/g = 16 : lc 0,045 → 0,0225 → 0,01125 déplace la moyenne EF de −0,130 % puis −0,050 % ; lc 0,0225 validé) ; aux autres g par `prod_sweep_b0g_meshcheck.py`, lc 0,045 → 0,0225 sur les douze positions à b₀/g = 12 (−0,123 %, au-delà du critère 0,1 % → ligne promue à lc 0,0225 par `prod_sweep_b0g_meshrow.py`) et 4 (−0,074 %), à φ = 0 seul à b₀/g = 2 (−0,058 %, 1,2 × 10⁶ nœuds) ; nominal : −0,082 % (Table 2) | `prod_sweep_b0g.py` (G5), `prod_sweep_b0g_meshcheck.py`, `prod_sweep_b0g_meshrow.py` | `prod_sweep_b0g_meshcheck_{results.json,out.txt}` ; en-tête et Table 4 de `prod_sweep_b0g_out.txt` | 19–20 sept. |
