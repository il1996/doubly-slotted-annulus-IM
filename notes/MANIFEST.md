...
| Fig. 7 et §IV-C (dernier paragraphe), balayage du rapport b₀/g sur la chaîne linéaire : g seul varie (R_s fixe, R_r = R_s − g), b₀ = 2,000 mm aux deux surfaces, pavage (33,16), chapeau symétrique, N_h = 8192, cavités Q rechargées de `cavity_Q.pkl` ; b₀/g ∈ {2, 4, 8,2 (nominal relu de `prod_fem_results.json` / `prod_op_results.json`), 12, 16} ; à chaque g les douze positions rotor : EF formulation A (lc 0,045, comparateur forme fermée recalculé), produit de Carter (formes approchée et conforme exacte), opérateur Φ_O = 0 et opérateur + cavités (comparateur assemblé recalculé) ; gardes G1–G5 (tolérance 0 sur les chiffres imprimés ; G5 : au g le plus fin, lc/2 déplace la moyenne EF de moins de 0,1 % — à b₀/g = 16 le maillage de production échoue (−0,13 %) et la ligne EF est rapportée à lc 0,0225, validé par 0,01125 (−0,05 %)) ; contrôle lc/2 aux autres g par `prod_sweep_b0g_meshcheck.py` (douze positions à 12 et 4, φ = 0 seul à 2) | `prod_sweep_b0g.py` → `FIG7_B0G_SWEEP.py` ; `prod_sweep_b0g_meshcheck.py` | `prod_sweep_b0g_results.json`, `prod_sweep_b0g_out.txt` (en-tête : date, durées, gardes) ; `prod_sweep_b0g_meshcheck_{results.json,out.txt}` ; `code/article/figures/fig7_b0g_sweep.{pdf,png}` | 19 sept. |
